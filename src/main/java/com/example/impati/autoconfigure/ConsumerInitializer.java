...
public class ConsumerInitializer {
    ...
    void init() {
        ...
        List<Channel> channels = channelRegistration.getChannels().values().stream().toList();
        channelSubscriber.subscribe(Client.getInstance(), channels);
        channels.forEach(messagingSystemPoller::listen);
    }
}
